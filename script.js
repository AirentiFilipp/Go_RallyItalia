/* ═══════════════════════════════════════════════════════
   ...
   Comunicazione con l'API PHP via fetch()
═══════════════════════════════════════════════════════ */
...
// ── State ───────────────────────────────────────────────
let isAdmin      = false;
let currentRace  = null;   // oggetto gara corrente
let currentStage = null;   // id prova speciale corrente
let currentCat   = 'Rally2';
let deleteTarget = null;   // { type: 'time'|'race', id }
let allRaces     = [];
...
function showPage(page, data = null) {
  ...
  if (page === 'home') {
    ...
    loadRacePage(data);   // data = raceId
  } else if (page === 'insert') {
    document.getElementById('navInsert').classList.add('active');
    prepareInsertForm(data);  // data = raceId (opzionale)
  }
}
...
function renderCalendar() {
  ...
  tbody.innerHTML = allRaces.map(r => {
    ...
    if (r.status === 'live') {
      actions = `
        <button class="btn btn-primary btn-sm" onclick="showPage('race',${r.id})">🏆 Classifica</button>
...
async function loadRacePage(raceId, resetCat = true) {
  ...
  if (currentRace.stages.length) {
    currentStage = currentRace.stages[0].id;
  }
...
function renderLeaderboard(times) {
  ...
  // Ordina per tempo totale (già ordinato dal DB, ma facciamo sicuro)
  ...
  tbody.innerHTML = approved.map((t, i) => {
    ...
    return `<tr>
      ...
      <td>
        <div class="driver-name">${esc(t.driver)}</div>
        <div class="driver-info">${esc(t.weather)}</div>
      </td>
...
async function insertTime() {
  ...
  try {
    const result = await api('POST', '/times', {
      ...
      time_min: min, time_sec: sec, time_dec: dec
    });
    toast(result.message || '✅ Tempo salvato!');
    resetInsertForm();
    if (currentRace?.id === raceId) {
      setTimeout(() => showPage('race', raceId), 900);
    }
  } catch(e) {
    toast(e.message, true);
  } finally {
...
function toggleAddRaceForm() {
  document.getElementById('addRaceForm').classList.toggle('open');
}
...
async function addRace() {
  ...
  try {
    ...
    document.getElementById('addRaceForm').classList.remove('open');
    await loadCalendar();
  } catch(e) { toast(e.message, true); }
}
...
// Enter nel campo password
document.addEventListener('keydown', e => {
  if (e.key === 'Enter' && document.getElementById('adminModal').classList.contains('open')) {
    doAdminLogin();
  }
});
...
function fmtTime(t) {
  const m = parseInt(t.time_min);
  const s = String(parseInt(t.time_sec)).padStart(2, '0');
  const d = parseInt(t.time_dec);
  return `${m}:${s}.${d}`;
}
